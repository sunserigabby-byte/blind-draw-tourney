...
/**
 * Sunny Sports Performance – Blind Draw Tourney (tabbed build, with Quads playoffs)
 *
 * TABS
 *  - Revco Doubles
 *  - Revco Quads
 ...
 * ✅ Live Leaderboard (Guys & Girls) with W/L/PD (to 21+, win by 2, no cap)
 * ✅ Autosave (rosters, matches, brackets) to localStorage
 * ✅ Playoff Builder: split upper/lower, pair within buckets, seeded by combined W then PD
 ...
 *    - Seeding mode:
 *       • Combined leaderboard (top X players overall), or
 *       • Split by gender (top N guys + top N girls)
 *    - Builds teams with gender priority:
 *       • Prefer 2 guys + 2 girls
 *       • Then 3+1
 *       • Then all-one-gender only if needed
 *    - Ensures at most 2 triples teams if numbers demand it
 *    - Seeds teams by combined W + PD and builds a single-elim bracket
 */
...
interface Team {
  id: string;
  name: string;
  members: string[];  // doubles = 2, quads = 4, triples = 3
  seed: number;
  division: PlayDiv;
}
...
// Court pools for playoffs (doubles brackets)
...
function QuadsRoundGenerator({
  ...
  const [strictOpponents, setStrictOpponents] = useState(true);
  const [roundsToGen, setRoundsToGen] = useState(1);
  const [startCourt, setStartCourt] = useState(1);
  ...
  const MAX_TEAMMATE_REPEAT = 2;
  const MAX_OPP_REPEAT      = 2;

  type CountMap = Map<string, Map<string, number>>;

  const addCount = (map:CountMap, a:string, b:string)=>{
    const A = slug(a), B = slug(b);
    if (A === B) return;
  ...
  // Teammate counts: how many times two players have been on the same team
  const buildTeammateMap = (history:QuadsMatchRow[]): CountMap=>{
  ...
  // Opponent counts: how many times two players have opposed each other
  const buildOpponentMap = (history:QuadsMatchRow[]): CountMap=>{
    const mp: CountMap = new Map();
    for (const m of history) {
      const t1 = m.t1;
      const t2 = m.t2;
      for (const a of t1) for (const b of t2) addCount(mp, a, b);
    }
    return mp;
  };

  const registerTeammates = (teammateMap:CountMap, team:string[])=>{
    for (let i=0;i<team.length;i++){
      for (let j=i+1;j<team.length;j++){
        addCount(teammateMap, team[i], team[j]);
      }
    }
  };

  const registerOpponents = (opponentMap:CountMap, teamA:string[], teamB:string[])=>{
    for (const a of teamA) {
      for (const b of teamB) {
        addCount(opponentMap, a, b);
      }
    }
  };

  const haventOpposedTeam = (opponentMap:CountMap, teamA:string[], teamB:string[])=>{
    if (!strictOpponents) return true;
    for (const a of teamA){
      const row = opponentMap.get(slug(a));
      if (!row) continue;
      for (const b of teamB){
        const count = row.get(slug(b)) || 0;
        if (count >= MAX_OPP_REPEAT) return false;
      }
    }
    return true;
  };

  // Decide how many quads vs triples (n = 4a + 3b, minimize b)
  const computeTeamSizes = (totalPlayers:number): number[]=>{
    if (totalPlayers <= 0) return [];
    let quads = 0, triples = 0;
    ...
    if (mod === 0) {
      ...
      // Try 3 triples if possible (n = 4a + 9)
      if (n >= 13) {
        triples = 3;
        quads   = (n - 3*3) / 4;
      } else {
        // small weird cases – fall back to 1 triple, rest quads where possible
        triples = 1;
        quads   = Math.max(0, Math.floor((n - 3)/4));
      }
    } else if (mod === 2) {
      // typically 2 triples: n = 4a + 6
      if (n >= 6) {
        triples = 2;
        quads   = (n - 2*3) / 4;
      } else {
        // fallback: 1 triple, whatever remains
        triples = 1;
        quads   = Math.max(0, Math.floor((n - 3)/4));
      }
    } else if (mod === 3) {
    ...
    for (let i=0;i<triples;i++) sizes.push(3);
    return sizes;
  };

  // Decide gender composition for a quad (priority: 2/2 → 3/1 → 4/0)
  const chooseQuadComposition = (remainingM:number, remainingF:number, teamsLeft:number)=>{
    const size = 4;

    // Last team: just take everybody left
    if (teamsLeft === 1) {
      return { needM: remainingM, needF: remainingF };
    }
    ...
    // If one gender is clearly majority, lean 3+1 in that direction
    ...
    // If we're out of a gender, we have to go 4 of the other
    ...
    // Fallbacks – still try to keep mix if we can
  ...
  // Decide gender composition for a triple (priority: 2/1 → 3/0)
  const chooseTripleComposition = (remainingM:number, remainingF:number, teamsLeft:number)=>{
    ...
    // Otherwise: 3 of whichever is left
  ...
  // Pick a player from a pool while trying to avoid > MAX_TEAMMATE_REPEAT with current teammates
  const pickPlayerWithTeammateLimit = (
    pool:string[],
    currentTeam:string[],
    teammateMap:CountMap,
  ): string | undefined=>{
    ...
    for (let i=0;i<pool.length;i++){
      const cand = pool[i];
      let ok = true;
      for (const mate of currentTeam){
        const row = teammateMap.get(slug(cand));
        const count = row?.get(slug(mate)) || 0;
        if (count >= MAX_TEAMMATE_REPEAT) {
          ok = false;
          break;
        }
      }
      if (ok) { candidateIdx = i; break; }
    }
    if (candidateIdx === -1) candidateIdx = 0; // if no "clean" option, just take first
    const [chosen] = pool.splice(candidateIdx, 1);
    return chosen;
  };

  function buildRound(roundIdx:number, history:QuadsMatchRow[]){
    const allGuys  = shuffle([...guys]);
    const allGirls = shuffle([...girls]);

    const totalPlayers = allGuys.length + allGirls.length;
    if (totalPlayers < 6) {
      // too few players – fall back: one match of whatever we have
      if (totalPlayers === 0) return [] as QuadsMatchRow[];
      const teamA:string[] = [];
      const teamB:string[] = [];
      const pool = shuffle([...allGuys, ...allGirls]);
      while (pool.length && teamA.length < 4) teamA.push(pool.shift()!);
      while (pool.length && teamB.length < 4) teamB.push(pool.shift()!);
      const row:QuadsMatchRow = {
        id: `${roundIdx}-${startCourt}-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,
        round: roundIdx,
        court: startCourt,
        t1: teamA,
        t2: teamB,
        isTriple1: teamA.length === 3,
        isTriple2: teamB.length === 3,
        scoreText: '',
      };
      return [row];
    }

    // How many quads vs triples we want (3s only when n % 4 !== 0)
    const teamSizes = computeTeamSizes(totalPlayers); // array of 4s and 3s
    const teammateMap = buildTeammateMap(history);
    const opponentMap = buildOpponentMap(history);

    let remainingGuys  = allGuys.slice();
    let remainingGirls = allGirls.slice();

    const teams: { members:string[]; isTriple:boolean }[] = [];

    // Build each team (respecting gender-priority and teammate history "if possible")
    for (let t=0; t<teamSizes.length; t++){
      const size = teamSizes[t];
      const teamsLeft = teamSizes.length - t;

      const remainingM = remainingGuys.length;
      const remainingF = remainingGirls.length;

      const comp = size === 4
        ? chooseQuadComposition(remainingM, remainingF, teamsLeft)
        : chooseTripleComposition(remainingM, remainingF, teamsLeft);

      let needM = comp.needM;
      let needF = comp.needF;

      const members:string[] = [];

      while (members.length < size && (remainingGuys.length || remainingGirls.length)) {
        // Decide which gender to draw from next
        let fromGuys = false;
        if (needM > 0 && remainingGuys.length) fromGuys = true;
        else if (needF > 0 && remainingGirls.length) fromGuys = false;
        else if (remainingGuys.length && !remainingGirls.length) fromGuys = true;
        else if (remainingGirls.length && !remainingGuys.length) fromGuys = false;
        else {
          // both available but no explicit needM/needF – pick whichever has more
          fromGuys = remainingGuys.length >= remainingGirls.length;
        }

        const pool = fromGuys ? remainingGuys : remainingGirls;
        const chosen = pickPlayerWithTeammateLimit(pool, members, teammateMap);
        if (!chosen) break;
        members.push(chosen);
        if (fromGuys && needM>0) needM--;
        if (!fromGuys && needF>0) needF--;
      }

      // Register teammates so later teams in this same round see updated history
      registerTeammates(teammateMap, members);
      teams.push({ members, isTriple: size === 3 });
    }

    // If somehow we still have leftover players (paranoia), stuff them into the last team(s)
    let leftovers = [...remainingGuys, ...remainingGirls];
    if (leftovers.length) {
      for (const name of leftovers) {
        let target = teams[teams.length-1];
        target.members.push(name);
      }
      leftovers = [];
    }

    // Pair teams into matches, 2 teams per court, avoid repeat opponents > 2 "if possible"
    ...
    while (teamList.length >= 2) {
      ...
      // Update opponent map so later pairings in this same round respect limits
      registerOpponents(opponentMap, a.members, b.members);
    ...
    // If one team is left unpaired (odd number of teams), just leave them out this round
    return made;
  }
  ...
  return (
    <section className="bg-white/90 backdrop-blur rounded-xl shadow ring-1 ring-slate-200 p-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-3 text-[12px] flex-wrap">
          <label className="flex items-center gap-1">
            <input
              type="checkbox"
              checked={strictOpponents}
              onChange={(e)=>setStrictOpponents(e.target.checked)}
            />
            Limit repeat opponents (&le; 2 if possible)
          </label>
          <label className="flex items-center gap-1">
            Rounds
            <input
      ...
      <p className="text-[11px] text-slate-500 mt-2">
        Uses everyone every round. If total players is divisible by 4, you&apos;ll get only quads. Triples are used only
        when needed, and the engine prioritizes 2 guys / 2 girls, then 3+1 mixes before any all-one-gender team. It also
        tries to keep teammates and opponents from repeating more than twice when possible.
      </p>
    </section>
  );
}

/* ========================= QUADS: Standings helper ========================= */
...
type QuadsBucket = {
  ...
  L: number;
  PD: number;
  gender: 'guy'|'girl'|'unknown';
};

function computeQuadsStandings(matches:QuadsMatchRow[], guysText:string, girlsText:string){
  const guysList = Array.from(new Set((guysText||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean)));
  const girlsList= Array.from(new Set((girlsText||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean)));
  const guysSet  = new Set(guysList.map(slug));
  const girlsSet = new Set(girlsList.map(slug));

  const all = new Map<string, QuadsBucket>();
  const ensure = (name:string, genderHint:'guy'|'girl'|'unknown')=>{
    const existing = all.get(name);
    if(existing) return existing;
    const g: 'guy'|'girl'|'unknown' =
      genderHint !== 'unknown' ? genderHint :
      guysSet.has(slug(name)) ? 'guy' :
      girlsSet.has(slug(name)) ? 'girl' : 'unknown';
    const row:QuadsBucket = { name, W:0, L:0, PD:0, gender:g };
    all.set(name, row);
    return row;
  };

  // Pre-seed from rosters so zero-game players show
  for(const n of guysList) ensure(n, 'guy');
  for(const n of girlsList) ensure(n, 'girl');
  ...
  for(const m of matches){
    ...
    const apply = (name:string, won:boolean)=>{
      const genderHint: 'guy'|'girl'|'unknown' =
        guysSet.has(slug(name)) ? 'guy' :
        girlsSet.has(slug(name)) ? 'girl' : 'unknown';
      const row = ensure(name, genderHint);
      if(won){ row.W++; row.PD += diff; } else { row.L++; row.PD -= diff; }
    };
  ...
  const sortFn = (a:QuadsBucket,b:QuadsBucket)=> b.W-a.W || b.PD-a.PD || a.name.localeCompare(b.name);
  const allRows = Array.from(all.values()).sort(sortFn);
  const guysRows = allRows.filter(r => r.gender==='guy' || guysSet.has(slug(r.name)));
  const girlsRows = allRows.filter(r => r.gender==='girl' || girlsSet.has(slug(r.name)));
...
function QuadsLeaderboard({
  ...
  const { guysRows, girlsRows } = useMemo(
    ()=> computeQuadsStandings(matches, guysText, girlsText),
    [matches, guysText, girlsText],
  );

  const Table = ({title, rows}:{title:string; rows:QuadsBucket[]})=> (
    <section className="bg-white/95 backdrop-blur rounded-xl shadow ring-1 ring-slate-200 p-4">
      <h3 className="text-[15px] font-semibold text-sky-800 mb-2">{title}</h3>
      <div className="overflow-x-auto">
        <table className="min-w-full text-[13px]">
...
/* ========================= QUADS: Playoff Builder + Bracket ========================= */

type QuadsSeedMode = 'COMBINED' | 'SPLIT';
...
function QuadsPlayoffBuilder({
  ...
  const [seedMode, setSeedMode] = useState<QuadsSeedMode>('COMBINED');
  const [combinedCount, setCombinedCount] = useState<number>(16);
  const [perGenderCount, setPerGenderCount] = useState<number>(4);
  const [error, setError] = useState<string>('');

  // Helper to build quads/triples teams from selected players
  function buildTeamsFromPlayers(players:QuadsBucket[]): Team[] {
    if(players.length < 4) return [];

    // Keep original order as seed reference (already sorted by W/PD)
    type P = QuadsBucket & { index:number };
    const withIndex: P[] = players.map((p,idx)=> ({...p, index:idx}));

    const guysQueue: P[] = withIndex.filter(p=> p.gender==='guy');
    const girlsQueue: P[] = withIndex.filter(p=> p.gender==='girl');
    const unknownQueue: P[] = withIndex.filter(p=> p.gender==='unknown');

    // If any unknowns, push them to the smaller gender bucket to help balance
    while(unknownQueue.length){
      const u = unknownQueue.shift()!;
      if(guysQueue.length <= girlsQueue.length) guysQueue.push(u);
      else girlsQueue.push(u);
    }

    const takeFromDeque = (queue:P[], count:number): P[]=>{
      const res:P[] = [];
      let fromFront = true;
      while(count>0 && queue.length>0){
        res.push(fromFront? queue.shift()!:queue.pop()!);
        fromFront = !fromFront;
        count--;
      }
      return res;
    ...
    const N = guysQueue.length + girlsQueue.length;
    const maxTeams = Math.floor(N/4);
    let teamsToMake = maxTeams;
    for(let t=maxTeams;t>=0;t--){
      const leftover = N - 4*t;
    ...
    const pickPattern = (gLeft:number, fLeft:number) => {
      const patterns = [
        {g:2, f:2},
        {g:3, f:1},
        {g:1, f:3},
      ...
      for(const p of patterns){
        if(gLeft>=p.g && fLeft>=p.f && gLeft+fLeft>=p.g+p.f) return p;
      }
      // fallback: whatever we can
      const totalLeft = gLeft + fLeft;
      const need = Math.min(4, totalLeft);
      const gNeed = Math.min(gLeft, need);
      const fNeed = need - gNeed;
      return {g:gNeed, f:fNeed};
    };

    // Build quads first
    for(let t=0;t<teamsToMake;t++){
      const gLeft = guysQueue.length;
      const fLeft = girlsQueue.length;
      const pattern = pickPattern(gLeft, fLeft);
      const gPick = takeFromDeque(guysQueue, pattern.g);
      const fPick = takeFromDeque(girlsQueue, pattern.f);
      let members = [...gPick, ...fPick];

      // If somehow <4, fill from the larger gender queue
      while(members.length < 4 && (guysQueue.length || girlsQueue.length)){
        if(guysQueue.length >= girlsQueue.length && guysQueue.length){
          members.push(...takeFromDeque(guysQueue,1));
        } else if(girlsQueue.length){
          members.push(...takeFromDeque(girlsQueue,1));
        } else break;
      }

      const name = members.map(m=>m.name).join(' · ');
      teams.push({
        id:`QTEAM-${t+1}-${slug(name)}`,
        name,
    ...
    // Leftover → triples (0, 3 or 6 players)
    const leftovers: P[] = [...guysQueue, ...girlsQueue];
    let gL = guysQueue.length;
    let fL = girlsQueue.length;
    ...
    let idx = 0;
    while(idx < leftovers.length){
      if(idx + 3 > leftovers.length) break;
      const remaining = leftovers.slice(idx);
      gL = remaining.filter(p=>p.gender==='guy').length;
      fL = remaining.filter(p=>p.gender==='girl').length;

      let pat = triplePatterns.find(p=> gL>=p.g && fL>=p.f);
      if(!pat){
        const totalLeft = remaining.length;
        const need = Math.min(3, totalLeft);
        const gNeed = Math.min(gL, need);
        const fNeed = need - gNeed;
        pat = {g:gNeed, f:fNeed};
      }

      const teamMembers:P[] = [];
      const takeFromRemaining = (gender:'guy'|'girl', count:number)=>{
        for(let i=0;i<remaining.length && count>0;i++){
          const p = remaining[i];
          if(p.gender===gender){
            teamMembers.push(p);
      ...
      takeFromRemaining('guy', pat.g);
      takeFromRemaining('girl', pat.f);
      while(teamMembers.length<3 && remaining.length){
        teamMembers.push(remaining.shift()!);
      }

      // rebuild leftovers from remaining for next loop
      leftovers.length = 0;
      leftovers.push(...remaining);
      idx = 0;

      const name = teamMembers.map(m=>m.name).join(' · ');
      teams.push({
        id:`QTEAM-T-${teams.length+1}-${slug(name)}`,
        name,
    ...
    // Re-seed teams by combined W+PD
    const statsMap = new Map<string, QuadsBucket>();
    for(const row of allRows){
      statsMap.set(row.name, row);
    }

    const scoreTeam = (t:Team)=>{
      let W = 0, PD = 0;
    ...
    teams.sort((A,B)=>{
      const a = scoreTeam(A), b = scoreTeam(B);
      return (b.W - a.W) || (b.PD - a.PD) || A.name.localeCompare(B.name);
    });
    teams.forEach((t,i)=>{ t.seed = i+1; t.id = `QTEAM-${t.seed}-${slug(t.name)}`; });
  ...
  function handleBuild(){
    setError('');
    let selected: QuadsBucket[] = [];

    if(seedMode==='COMBINED'){
      const maxN = allRows.length;
      const n = Math.min(clampN(combinedCount, 4), maxN);
      selected = allRows.slice(0, n);
    }else{
      const gN = Math.min(clampN(perGenderCount, 1), guysRows.length);
      const fN = Math.min(clampN(perGenderCount, 1), girlsRows.length);
      if(gN===0 && fN===0){
        setError('Not enough players in guys/girls leaderboards.');
        return;
      }
      const combined = [...guysRows.slice(0,gN), ...girlsRows.slice(0,fN)];
      // sort combined by W/PD for fair seeding
      selected = combined.slice().sort((a,b)=> b.W-a.W || b.PD-a.PD || a.name.localeCompare(b.name));
    }

    if(selected.length < 4){
      setError('Need at least 4 players to build a quads playoff.');
      return;
    }

    const teams = buildTeamsFromPlayers(selected);
    if(!teams.length){
      setError('Could not form any valid teams from the selected players.');
      return;
    }
  ...
  return (
    <section className="bg-white/95 backdrop-blur rounded-xl shadow ring-1 ring-slate-200 p-4">
      <h2 className="text-[16px] font-semibold text-sky-800 mb-2">Playoff Setup (Quads)</h2>
      <p className="text-[11px] text-slate-500 mb-3">
        Build single-elim quads playoffs from pool standings. Teams are seeded by combined W and point differential.
        Engine prioritizes 2 guys + 2 girls per team, then 3+1, then all-one-gender if needed.
      </p>

      <div className="space-y-3 text-[12px]">
        <div className="flex flex-wrap gap-3 items-center">
          <span className="font-medium text-slate-700">Seeding mode:</span>
          <label className="flex items-center gap-1">
            <input
              type="radio"
              name="q-seed-mode"
              value="COMBINED"
              checked={seedMode==='COMBINED'}
              onChange={()=>setSeedMode('COMBINED')}
            />
            Combined leaderboard (all players)
          </label>
          <label className="flex items-center gap-1">
            <input
              type="radio"
              name="q-seed-mode"
              value="SPLIT"
              checked={seedMode==='SPLIT'}
              onChange={()=>setSeedMode('SPLIT')}
            />
            Split by gender (top N guys &amp; girls)
          </label>
        </div>
        ...
        {seedMode==='COMBINED' ? (
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-2">
              Total players from combined leaderboard
              <input
                type="number"
                min={4}
                className="w-20 border rounded px-2 py-1"
                value={combinedCount}
                onChange={(e)=>setCombinedCount(clampN(+e.target.value||4,4))}
              />
            </label>
            <span className="text-[11px] text-slate-500">
              Tip: choose a multiple of 4, or a value that leaves 3 or 6 extra players (for up to two triples teams).
            </span>
          </div>
        ) : (
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-2">
              ...
              <input
                ...
                min={1}
                className="w-16 border rounded px-2 py-1"
                value={perGenderCount}
                onChange={(e)=>setPerGenderCount(clampN(+e.target.value||1,1))}
              />
            </label>
            <span className="text-slate-500 text-[11px]">and</span>
            <span className="text-[12px] text-slate-700">
              Top N girls (same N). Actual counts are limited by leaderboard size.
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-[11px] text-slate-500">
            Bracket will appear below. You can edit scores there and winners will auto-advance.
          </span>
        </div>
      </div>
    </section>
...
function QuadsBracketView({
  ...
  const cfg = buildVisualColumns(brackets, 'UPPER');
  const cols = cfg.cols;

  if(!cols.length){
    return (
      <section className="bg-white/95 backdrop-blur rounded-2xl shadow-lg ring-1 ring-sky-200 p-6">
        <h2 className="text-[20px] font-bold text-sky-900 mb-2 tracking-tight">Quads Playoff Bracket</h2>
        <p className="text-[11px] text-slate-500">
          Build the bracket above to see the single-elimination layout here.
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="bg-white/95 backdrop-blur rounded-2xl shadow-lg ring-1 ring-sky-200 p-6">
      <h2 className="text-[20px] font-bold text-sky-900 mb-2 tracking-tight">Quads Playoff Bracket</h2>
      <p className="text-[11px] text-slate-500 mb-4">
        Single-elim quads bracket built from pool standings. Enter scores to advance winners.
      </p>
      <div className="overflow-x-auto">
        <div
          className="grid gap-6"
...
export default function BlindDrawTourneyApp(){
  ...
  useEffect(()=>{
    try{
      ...
      if(data.activeTab==='QUADS' || data.activeTab==='DOUBLES') setActiveTab(data.activeTab);
    }catch{/* ignore */}
  },[]);
    ...
    <main className="min-h-screen bg-gradient-to-b from-sky-800 via-sky-700 to-sky-500 text-slate-800 antialiased">
      <header className="sticky top-0 z-10 bg-sky-900/90 backdrop-blur border-b border-sky-700 shadow-sm">
        ...
        <div className="border-t border-sky-700 bg-sky-900/80">
          <div className="max-w-6xl mx-auto px-4 py-1.5 flex gap-2 text-[13px]">
            <button
              className={
                "px-3 py-1 rounded-t-md border-b-2 " +
                (activeTab==='DOUBLES'
                  ? "bg-white text-sky-900 border-sky-400"
                  : "bg-transparent text-sky-100/80 border-transparent hover:bg-sky-800/60")
              }
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        {activeTab==='DOUBLES' ? (
          <>
            {/* Leaderboard first */}
            <Leaderboard matches={dMatches} guysText={dGuysText} girlsText={dGirlsText} />

            {/* Matches view */}
            <MatchesView matches={dMatches} setMatches={setDMatches} />
            ...
            <section className="bg-white/95 backdrop-blur rounded-xl shadow ring-1 ring-slate-200 p-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <LinedTextarea
                  ...
                  label="Guys – Doubles"
                  value={dGuysText}
                  onChange={(e)=>setDGuysText(e.target.value)}
                ...
                <LinedTextarea
                  ...
                  label="Girls – Doubles"
                  value={dGirlsText}
                  onChange={(e)=>setDGirlsText(e.target.value)}
                  placeholder="One name per line"
                />
              </div>
            </section>
            ...
            <PlayoffBuilder
              ...
              guysText={dGuysText}
              girlsText={dGirlsText}
              setBrackets={setDBrackets}
            />
            <BracketView brackets={dBrackets} setBrackets={setDBrackets} />
          </>
        ) : (
          <>
            {/* QUADS TAB CONTENT */}
            ...
            <QuadsRoundGenerator
              ...
              matches={qMatches}
              setMatches={setQMatches}
            />

            <section className="bg-white/95 backdrop-blur rounded-xl shadow ring-1 ring-slate-200 p-4">
              <h2 className="text-[16px] font-semibold text-sky-800 mb-2">Players (Quads)</h2>
              <p className="text-[11px] text-slate-500 mb-2">
                These rosters are <strong>separate</strong> from the doubles tab so you can run quads-only events or mix.
              </p>
              <div className="grid md:grid-cols-2 gap-4">
                <LinedTextarea
                  id="q-guys"
                  label="Guys – Quads"
                  value={qGuysText}
                  onChange={(e)=>setQGuysText(e.target.value)}
                  placeholder="One name per line"
                />
                <LinedTextarea
                  id="q-girls"
                  label="Girls – Quads"
                  value={qGirlsText}
                  onChange={(e)=>setQGirlsText(e.target.value)}
                  placeholder="One name per line"
                />
              </div>
            </section>
            ...
            />
            <QuadsBracketView brackets={qBrackets} setBrackets={setQBrackets} />
          </>
        )}
        ...
        <section className="bg-white/80 rounded-lg p-3 text-[11px] text-slate-600">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span>Autosave is on for both tabs. Export/Import can be added later if you want CSV backups.</span>
          </div>
        </section>
      </div>
    </main>
