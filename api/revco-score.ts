...
// single Revco Quads match score. Unlike /api/state, this does NOT require
// the real admin key — instead it checks the submitted scorerPin against the
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const div = state?.revcobd?.[division];
    ...
    if (!found) return res.status(404).json({ ok: false, error: "Match not found" });

    await redis.set(KEY, state);
    return res.status(200).json({ ok: true });
  } catch (err: any) {
    console.error("api/revco-score error:", err);
